...
/**
  ...
  public function createAction() {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $data = array(
        ...
        'discount_sign' => $this->input->post('discount_sign', true),
        'total_price' => $this->input->post('total_price', TRUE),
        'sample_collection_time' => $this->input->post('sample_collection_time', TRUE),
        'appointment_date' => date('Y-m-d'),
        'doctor_id' => $this->input->post('doctor_ref_by', TRUE),
        'appointment_status' => $this->input->post('appointment_status', TRUE),
        'report_doc' => '',
        'payment_status' => $this->input->post('payment_status', TRUE),
      );
      $subTestName = NULL;
      $subTestIDs = $this->input->post('subtest');
      $result = $this->subtest_model->getDataByKey('id', $subTestIDs, 'IN');
      if (count($result) > 0) {
        foreach ($result as $res) {
          $subTestName[] = $res->subtest_name;
        }
        $subTestName = implode(', ', $subTestName);
      }
      $testResult = $this->tests_model->get_by_id($this->input->post('test'));
      $data['subtest_name'] = $subTestName;
      $data['subtest_ids'] = implode(',', $subTestIDs);
  ...
  public function update($id) {
    ...
    $this->load->model('doctors_model', 'doctors_m');
    if ($row) {
      $subsetOption[] = 'SELECT SUBTEST';
      $result = $this->subtest_model->get_all_subtest($row->test);
      if (count($result) > 0) {
        foreach ($result as $res) {
          $subsetOption[$res->id] = $res->subtest_name;
        }
      }
      ...
      $data = array(
        ...
        'doctor_id' => set_value('doctor_id', $row->doctor_id),
        ...
        'test_options' => $this->tests_m->getTestOptions(),
        'doctor_options' => $this->doctors_m->getDoctorOptions(),
        'discount_price' => set_value('discount_price', $row->discount_price),
        'subtest_options' => $subsetOption,
        'subtest' => set_value('discount_price', explode(',', $row->subtest_ids)),
        'discount_sign' => set_value('discount_sign', $row->discount_sign),
        'appScript' => "ADMIN.createAppointment.init()"
      );
  ...
  public function rules() {
    ...
    $this->form_validation->set_rules('subtest[]', 'subtest', 'trim|required|integer');
